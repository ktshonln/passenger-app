...
export default function BookingSuccessScreen() {
  ...
  useEffect(() => {
    if (booking?.id && !fromMyTrips) addBooking(booking);
    Animated.parallel([
        ...
        >
          ...
          <TouchableOpacity
            className={`rounded-2xl h-[54px] items-center justify-center ${
              fromMyTrips ? "bg-primary" : "border border-primary"
            }`}
            onPress={() =>
              fromMyTrips
                ? router.back()
          ...
          >
            <View className="flex-row items-center gap-2">
              <Ionicons
                name={fromMyTrips ? "arrow-back-outline" : "ticket-outline"}
                size={17}
                color={fromMyTrips ? "#FFFFFF" : "#0A4370"}
              />
              <Text
                className={`text-[15px] font-bold ${
                  fromMyTrips ? "text-white" : "text-primary"
                }`}
              >
                {fromMyTrips
                  ? "Back to My Trips"
